...
from app.utils.logger import click_logger

# noinspection PyMethodMayBeStatic
class ClickService:
    """Сервис игровой механики щелчков"""

    async def process_click(self, user: UserData) -> ClickResult:
        ...
        try:
            ...
            click_logger.exception(f"[CLICK] Reward updated failed | user_id={user_id} | reward={money}")
            raise

        new_money = user.money + money
        new_clicks = user.clicks + 1

        click_logger.info(f"[CLICK] Reward issued | user_id={user_id} | money={money} | total_clicks={new_clicks}")
