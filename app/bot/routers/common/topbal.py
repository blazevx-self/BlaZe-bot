...
async def top_command(message: Message, user: dict):
    data = await top_service.process_top(user=user)
    text = build_top_text(data)

    await message.reply(text, parse_mode='HTML', reply_markup=get_update_top_only_kb())
...
async def refresh_top_we(callback: CallbackQuery, user: dict):
    ...
    try:
        await callback.message.edit_text(text=text, parse_mode='HTML', reply_markup=get_top_ghoul_kb())
...
async def update_top_only(callback: CallbackQuery, user: dict):
    ...
    try:
        await callback.message.edit_text(text=text, parse_mode='HTML', reply_markup=get_update_top_only_kb())
        await callback.answer("Обновлён топчик", show_alert=False)
...
async def back_to_balance(callback: CallbackQuery, user: dict):
    ...
    await callback.message.edit_text(text=text, parse_mode='HTML', reply_markup=get_back_to_top_kb())
    await callback.answer()
